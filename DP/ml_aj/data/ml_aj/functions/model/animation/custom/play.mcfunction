...
$data modify storage ml_aj:logic data.temp.anim set value {name:"",anim:"$(name)",loop:$(loop),time:4,anim_time:0,measure:"t",command:"function ml_aj:model/animation/custom/play_end",event:"ml_aj.custom_anim"}
...
#Only needs stop if using loop mode 1, which is play once
data modify storage ml_aj:logic data.temp.anim.time set from storage ml_aj:logic data.temp.anim.anim_time
execute if data storage ml_aj:logic {data:{temp:{anim:{loop:1}}}} run function ml_ces:event/schedule/set_named with storage ml_aj:logic data.temp.anim
