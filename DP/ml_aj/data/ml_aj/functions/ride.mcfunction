## Ride a locator/camera attached to a model
## "id": The model id
...
$execute store result score @s ml_aj.temp as @e[tag=aj.rig_root] if data entity @s {item:{tag:{data:{badge:$(id)}}}} run function ml_aj:model/location/locator/ride with storage ml_aj:logic data.player.temp

tag @s remove ml_aj.locator_target

$execute if score @s ml_aj.temp matches 0 run tellraw @a[tag=debug] [{"text":"\n","color":"red"},{"text":"Failed to make the entity "},{"selector":"@s","color":"gold"},{"text":" ride the locator named "},{"text":"$(name)","color":"gold"},{"text":" for the model with id "},{"text":"$(id)","color":"gold"},{"text":".\n"}]
