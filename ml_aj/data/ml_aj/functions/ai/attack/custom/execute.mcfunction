...
## "entity": The target's UUID string or selector
...
$execute if data storage ml_aj:logic {data:{temp:{ai:{true:$(dir)}}}} run function ml_aj:model/vehicle/execute_pos {command:"execute anchored eyes positioned ^ ^ ^0.001 store result score #ml_temp ml_aj.temp as $(entity) if entity @s[distance=..$(distance)] run function ml_aj:ai/attack/execute_dir with storage ml_aj:logic data.temp.target"}
...
$execute if data storage ml_aj:logic {data:{temp:{ai:{true:$(dir)}}}} if entity @p[tag=ml_aj.attack_target] run tag @s[tag=!ml_aj.anim_attack_z] add ml_aj.rotate_target
execute on passengers if entity @s[tag=ml_aj.bone_head] run data modify entity @s transformation.right_rotation set value [0f,0f,0f,1f]
...
#Attack the entity at the impact time
...
execute if score @s ml_aj.temp matches ..15 as @e[tag=ml_aj.attack_target] at @s run function ml_aj:ai/attack/entity with storage ml_aj:logic data.temp.ai.anim
execute if score @s ml_aj.temp matches 16.. as @e[tag=ml_aj.attack_target] at @s run function ml_aj:ai/attack/entity_impact with storage ml_aj:logic data.temp.ai.anim
...
tag @e remove ml_aj.attack_target
