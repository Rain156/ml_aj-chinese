...
execute if entity @s[tag=ml_aj.anim_death] run return 1
execute if data entity @s {item:{tag:{data:{anims:{death:{time:0}}}}}} run return 0

function ml_aj:model/animation/clear_tags

tag @s add ml_aj.anim_death
...
data modify storage ml_aj:logic data.temp.anim.anim_time set from entity @s item.tag.data.anims.death.time
function ml_aj:model/animation/play_tween with storage ml_aj:logic data.temp.anim

#Set the stop since not looping
data modify storage ml_aj:logic data.temp.anim.time set from entity @s item.tag.data.anims.death.time
